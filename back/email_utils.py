...
def send_enrollment_email(recipient_email, student_name, course_name):
    ...
    if not all([config.SMTP_SERVER, config.SMTP_USER, config.SMTP_PASSWORD, config.SENDER_EMAIL]):
    ...
    You can find the course materials [here: link_to_course_materials] (Replace with actual link logic if available).
    ...
    try:
        ...
        if config.SMTP_USE_TLS:
            ...
            server.quit()
            logging.info(f"Enrollment email sent successfully to {recipient_email}")
            return True
         ...
         logging.error(f"SMTP Authentication Error sending email: {e}. Check SMTP credentials.")
         return False
